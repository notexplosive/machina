...
namespace Machina.Components
{
    class Hoverable : BaseComponent
    {
        ...
        public override void DebugDraw(SpriteBatch spriteBatch)
        {
            if (this.IsHovered)
            {
                spriteBatch.FillRectangle(this.boundingRect.Rect, new Color(Color.Blue, 0.25f), this.actor.progeny.Depth - 0.000001f);
            }

            if (this.debug_isHoveredFromCallbacks)
            {
                spriteBatch.DrawRectangle(this.boundingRect.Rect, Color.Orange, 2f, this.actor.progeny.Depth - 0.000002f);
            }
        }
