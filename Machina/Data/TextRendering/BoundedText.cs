        ...
        public List<RenderableText> GetRenderedText(Color textColor, int occludedCharactersCount = 0)
        {
            ...
            foreach (var row in this.bakedLayout.Rows)
            {
                foreach (var tokenNode in row)
                {
                    ...
                    var pendingRenderableText = new RenderableText(outputFragment.FontMetrics, outputFragment.Text, characterIndex, TotalAvailableRect.Location, textColor, tokenNode.Rectangle.Location.Negated());
